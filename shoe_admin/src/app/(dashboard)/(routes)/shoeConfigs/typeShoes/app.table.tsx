...
export default function TypeShoesTable() {
    ...
    const [selectedItemId, setSelectedItemId] = React.useState<number | null>(null); // State để lưu trữ id của hàng đang được chọnv

    const getData = async () => {
        const res = await shoeConfigAPI.getTypesConfig();
        console.log(res)
        if (res) {
            setData(res);
        }
    ...
    const handleAddDataConfig = async () => {
        try {
            ...
            await shoeConfigAPI.addShoesType(newData);
